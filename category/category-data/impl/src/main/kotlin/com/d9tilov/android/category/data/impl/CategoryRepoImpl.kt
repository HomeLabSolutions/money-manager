...
class CategoryRepoImpl
    @Inject
    constructor(
        private val categoryLocalSource: CategorySource,
    ) : CategoryRepo {
        override suspend fun createExpenseDefaultCategories() = categoryLocalSource.createExpenseDefaultCategories()

        override suspend fun createIncomeDefaultCategories() = categoryLocalSource.createIncomeDefaultCategories()
        ...
        override fun getCategoriesByType(type: TransactionType) = categoryLocalSource.getCategoriesByType(type)

        override fun getChildrenByParent(parentCategory: Category): Flow<List<Category>> =
            categoryLocalSource
                .getByParentId(parentCategory.id)
