...
class StatisticsViewModel @Inject constructor(private val transactionInteractor: TransactionInteractor) :
    ...
    var chartPeriod: StatisticsPeriod = StatisticsPeriod.DAY
        private set
    var periodPair: Pair<Long, Long> =
        Pair(currentDate().getStartOfDay().toMillis(), currentDate().getEndOfDay().toMillis())
        private set

    private val transactionsLiveData = MutableLiveData<List<TransactionChartModel>>()

    fun updatePeriod(period: StatisticsPeriod, periodPair: Pair<Long, Long>? = null) =
        viewModelScope.launch(Dispatchers.Main) {
            ...
            if (periodPair == null) {
                ...
                this@StatisticsViewModel.periodPair = periodPair
                transactionInteractor.getTransactionsGroupedByCategory(
                    TransactionType.EXPENSE,
                    periodPair.first.toLocalDate().getStartOfDay(),
