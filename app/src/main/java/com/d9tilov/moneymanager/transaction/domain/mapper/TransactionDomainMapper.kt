...
fun Transaction.toDataModel(): TransactionDataModel = TransactionDataModel(
    id, clientId, type, category.id, currencyCode, sum, usdSum, date, description, qrCode, isRegular
)

fun TransactionDataModel.toDomainModel(category: Category): Transaction =
    Transaction(id, clientId, type, category, currency, sum, usdSum, date, description, qrCode, isRegular)
