...
abstract class GradleFileValidationTask : DefaultTask() {
        ...
        Files.walkFileTree(
            ...
            object : SimpleFileVisitor<Path>() {
                ...
                override fun preVisitDirectory(dir: Path, attrs: BasicFileAttributes): FileVisitResult {
                    return if (dir.name == "build" || dir.name == ".gradle") FileVisitResult.SKIP_SUBTREE
    ...
    private fun shouldSkipFile(path: Path, rootPath: Path): Boolean {
        return if (path.parent == rootPath) true
        else { Files.lines(path).use { lines ->
                lines.map { it.trim() }
    ...
    private fun checkTransitiveInternalDependency(path: Path) {
        Files.lines(path).use { lines ->
            lines.map { it.trim() }
                ...
                .forEach { throw IllegalStateException("Transitive internal dependency $it\nin file $path is not allowed. Please, replace it with implementation") }
        }
    }
    ...
    private fun checkUnusedDependencies(path: Path) {
        ...
        val found: Boolean = Files.lines(path).use { lines ->
            lines.map { it.trim() }
    ...
    private companion object {
        ...
        val TRANSITIVE_DEPENDENCY_LIST = listOf(
            "api(project(\"",
            "debugApi(project(\"",
            "releaseApi(project(\"",
            "testApi(project(\"",
